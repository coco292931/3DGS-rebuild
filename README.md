...
展示用途的 Web 看板，一个进程同时提供后端 API 与页面，零第三方依赖（只用 Python 标准库）。

```bash
python tools/dashboard.py                 # 默认端口 8770，扫描 output/
...
| 栏位 | 内容 |
|---|---|
| 左 | 训练运行列表（正在写入的运行带绿点）+ 该运行的 checkpoint 列表 |
| 中 | 内嵌查看器，点 checkpoint 即实时预览（直接吃 `/model/...`，不需要先下载） |
| 右 | 指标卡片（PSNR/SSIM/高斯数/速度/耗时）、loss 曲线、训练日志尾部 |

**自动刷新**：每 4 秒轮询，训练进行中时曲线与指标会自动跟上，无需手动刷新。

**后端接口**：

| 路径 | 说明 |
|---|---|
| `GET /api/runs` | 所有运行及其状态、checkpoint 列表、是否活跃 |
| `GET /api/run/<name>` | 单个运行的日志（jsonl）与 summary.json |
| `GET /model/<name>/<file>` | PLY 模型文件（带路径越界检查） |
| `GET /preview/<name>/<file>` | 训练预览图 |
...
| 步骤 | 操作 |
|---|---|
| ① 导入视频 | 在「可导入的视频」里点一下 → 自动抽帧 + 求位姿 → 出现在数据集列表 |
| ② 训练 | 选中数据集 → 「用这个数据集训练」→ 选步数 / ckpt 间隔 / 预览间隔 |
| ③ 看结果 | 点任意 checkpoint 看 3D，或切「训练实时预览」看训练中的画面 |

**「可导入的视频」**自动扫 `Downloads/`、`3d-rebuild/`、`Videos/` 等目录下的视频，
标注大小与时间；点一下弹出导入表单（数据集名 / 抽帧频率 / 抽帧宽度），
导入即执行「ffmpeg 抽帧 → SIFT 特征 → 顺序匹配 → 增量式重建」，完成后自动出现在数据集列表。
实测：一段 36.7MB 的木桩视频，79 帧抽出、68 帧注册，耗时 35 秒。

**「数据集」**可点选，选中后展开该数据集可用的操作：已求位姿的给「用这个数据集训练」，
未求位姿的给「求相机位姿（SfM）」。（之前只列出来不给任何操作，是设计缺陷。）

**「新建训练」表单**里可以控制后端、训练步数、下采样，以及：

- **Checkpoint 间隔**：每 2000 / 5000 / 10000 步，或只保留最终模型
- **实时预览间隔**：每 500 / 2000 / 5000 步，或关闭

**「训练实时预览」标签页**显示训练过程中定期渲染的 GT 对比图，每 4 秒轮询一次，
训练进行中能眼看着画面从模糊变清晰。

### 启动

双击 `dashboard.bat`（自动打开浏览器）。
...
| 操作 | 效果 |
|---|---|
| **中键拖拽** | 旋转（Blender / SolidWorks 习惯） |
| **中键 + Shift** | 平移 |
| **中键 + Ctrl** | 缩放 |
| 左键拖拽 | 旋转（保留，方便没有中键的设备） |
| 滚轮 | 缩放 |
| WASD / QE | 平移 / 升降 |
| **R** | 重置视角（回到加载时的初始角度） |
| **B** | 循环切换背景色（天空蓝 / 深海灰 / 中性灰 / 纯白 / 暖灰） |
| URL `?fade=` | 近处渐隐半径系数，0 表示关闭 |

**加载反馈**：读取时显示进度条与已读字节数，解析与上传阶段显示状态文字。

**报错**：PLY 解析失败、文件为空、上传显存失败、HTTP 错误都会在页面上给出具体原因，
而不是静默无反应。
...
## 十三、为什么 loss 曲线波动大、checkpoint 效果不线性

这两个现象有**两个独立的原因**，都能用数据定位。

### 原因一：opacity reset 造成周期性冲击（解释 loss 锯齿）

`opacity reset` 每 N 步把所有高斯的不透明度压到 0.01 以下，逼着「又大又透明」的
垃圾高斯重新竞争。代价是**画面整体变淡、渲染立刻变差**，需要几百步才能恢复。

实测（6000 步训练，reset 每 1500 步，loss 每 50 步记录一次）：

```
跨越 reset 的区间:  平均 +31.79%   标准差 79.01%   范围 [-6.29%, +208.44%]
其他区间:           平均  -2.29%   标准差  6.76%   范围 [-34.61%, +19.25%]
```

**reset 区间平均是正的（loss 变差），其他区间平均是负的（正常下降）**——锯齿完全由它造成。
最夸张的一次：第 1500 步 reset，loss 从 0.0567 直接跳到 0.1445（+155%）。

这不是 bug，是设计如此。官方也这么做，只是官方跑 30000 步以上，有充足时间把每次
冲击消化掉；步数不够时，reset 的收益可能抵不上反复恢复的代价。

### 原因二：增密期的持续扰动（解释 checkpoint 不线性）

前一半步数里 `clone`/`split` 不断新增高斯，**每一个新分裂出来的高斯都是「没学好」的**，
它拉低整体指标，而它自己也需要几百步才收敛。结果是：**高斯数在变的时候，整体 loss 上不去。**

实测（同一份数据、30000 步）：

| 步数 | loss | 高斯数 | 阶段 |
|---|---|---|---|
| 5000 | 0.05279 | 60080 | 增密中 |
| 10000 | **0.05588** | 59869 | 增密中（**loss 反而涨了**） |
| 15000 | 0.05441 | 60102 | 增密刚停（densify-until=15000） |
| 20000 | 0.04027 | 60102 | 纯收敛 |
| 30000 | 0.03578 | 60102 | 纯收敛 |

对应的 checkpoint 实测 PSNR（26 个测试视角）：

| checkpoint | PSNR | 相对上一档 |
|---|---|---|
| iter_05000 | 23.980 | — |
| iter_10000 | 23.959 | **-0.021 dB（一万步白跑）** |
| iter_15000 | 26.664 | **+2.705 dB** |
| iter_20000 | 28.119 | +1.455 dB |
| iter_25000 | 28.558 | +0.439 dB |
| iter_30000 | 28.834 | +0.276 dB |

**15000 步那个 +2.7 dB 的突跃，正好是停止增密的时刻。** 这不是巧合：
停止增密后高斯数锁死，优化退化成纯粹的梯度下降，原先被扰动掩盖的收敛立刻显现。
...
0 ──────── 15000 ──────── 30000
  增密期：平台 + 抖动        收敛期：稳定下降
  （高斯数一直在变）        （高斯数锁死）
  叠加 opacity reset 的周期锯齿 ↑↓↑↓
```

所以「中间 checkpoint 效果不线性」是必然的：曲线的形状取决于你取的那个点
**离 reset 多远、在不在增密期**，而不只取决于步数。

### 实践上的三条建议

1. **不要按步数线性外推效果**。要挑最佳模型就用测试集实测，
   仓库里的 `tools/pick_best.py` 就是干这个的。
2. **增密期间不要频繁存 checkpoint**——那个阶段的权重没有保留价值，
   存了也是浪费磁盘和时间。把 ckpt 间隔设成 `densify-until` 之后开始才有意义。
3. **reset 间隔要和总步数匹配**。总步数少的时候（比如 6000 步），
   每 1500 步一次 reset 会让曲线一直在恢复期里挣扎；
   步数足够长时它才划算。折中值是总步数的 1/8 ~ 1/10。

---


### SfM 各阶段的并行效率（为什么 CPU 看起来没吃满）

实测（i5-13400F，10 核 16 线程；用同一批 1280×720 的抽帧图）：

| 阶段 | 单线程 | 默认（num_threads=-1） | 加速比 | 在总时长里的占比 |
|---|---|---|---|---|
| 抽帧 | — | 19 s（497 帧） | — | 6% |
| 特征提取 | 33.4 s / 60 帧 | **4.1 s / 60 帧** | **8.06x** | 12% |
| 顺序匹配 | 47.4 s / 120 帧 | **16.5 s / 120 帧** | **2.88x** | 32% |
| 增量式重建 | — | 154 s | **≈1x（本质串行）** | **49%** |

**结论：pycolmap 的默认配置已经用了所有核心**（`FeatureExtractionOptions.num_threads`
和 `FeatureMatchingOptions.num_threads` 默认都是 `-1`）。CPU 占用看起来上不去，
是因为**一半的时间花在增量式重建上，而这一步是串行的**——
它要逐个注册图像、每注册一张就跑一次局部 BA，后一张依赖前一张的结果，
没有可并行的空间。这是 COLMAP 算法的固有性质，不是配置问题。

补充两点实测结论：

- **GPU 跑 SIFT 不比 CPU 多线程快**：`use_gpu=True` 测得 7.97x，与 CPU 的 8.06x 基本持平。
  瓶颈不在特征计算，而在**图像解码与 I/O**（1280×720 PNG 每帧 ~69 ms）。
- 顺序匹配只有 2.88x，因为配对调度与几何验证存在串行段。

**因此真正能省时间的地方是减少输入量，不是加线程**：

1. 抽帧宽度从 1280 降到 960，解码量减少约 44%
2. 抽帧频率从 6 降到 4，总帧数减少三分之一（视差够用即可）
3. 用 `--max-image-size` 让 COLMAP 内部缩放（但解码仍按原始尺寸，收益有限）
