...
| 场景 | 数据来源 | 测试集 PSNR | SSIM | 高斯数 | 后端 |
|---|---|---|---|---|---|
| 合成场景 | 程序化生成（4 物体 + 棋盘地面） | **23.77 dB** | **0.929** | 24,075 | torch |
| 真实场景 · 木桩环绕 | 实拍视频 + SfM | **27.66 dB** | **0.900** | 60,102 | **cuda** |
| 真实场景 · 木桩特写 | 实拍视频 + SfM | **31.14 dB** | **0.950** | 59,941 | **cuda** |
| 真实场景（torch 后端对照） | 同第一段视频 | 22.51 dB | 0.849 | 31,830 | torch |

木桩特写的训练曲线（全程单调上升，没有出现任何崩溃）：

```
 5000 步  27.45 dB      20000 步  30.30 dB
10000 步  27.93 dB      25000 步  30.99 dB
15000 步  29.75 dB      30000 步  31.14 dB
```
...
**动机**：官方的 `split` 只在「梯度大」时触发，所以一个「变大但梯度小」（已拟合好）的高斯
永远不会被拆。官方 tile 渲染没有 K 上限，无所谓；本实现每像素只留 K 个，这类高斯会持续
推高片元数。于是加了一路 `--split-radius-frac`：屏幕半径超阈值就无条件拆（**拆不是剪**——
剪掉会让近处地面点成片消失，实测过，地面整个没了）。

**结果**（真实场景，其余参数完全相同）：

| 步数 | 强制拆分 (0.12) | 基线 | 片元数 拆分/基线 |
|---|---|---|---|
| 1200 | 19.72 dB | **21.74 dB** | 5.7M / 7.3M |
| 1500 | 20.84 dB | **22.51 dB** | 5.7M / 8.9M |
| 1800 | 20.26 dB | ~21.6 dB | 8.8M / 11.8M |

片元数确实降了约 25%，**但 PSNR 全程落后 1.5~2 dB**，且高斯数一直顶在 `--max-gaussians` 上限。

**结论：方向错误，已否决。** 推理错在把「片元多」当成了病因。这些大高斯在正确的位置、
正确的大小，已经收敛好了；拆开等于破坏已收敛的结构，让两个新高斯在学好的区域重新竞争。
K 截断真正丢掉的是**被它们挡住的远处贡献**——那是「K 有上限」这件事本身的问题，
不是「高斯太大」的问题。**解法只能是消除 K 上限（tile 分块 / CUDA 后端），不是控制高斯大小。**

该参数默认 0（关闭），保留代码作为实验记录。

**一个容易踩的坑**：验证这类「是否撞上 K 上限」的改动，窗口必须够长。
1200 步时片元才 7.3M（640×360 → 31.6 个/像素，K=64 还有一倍余量），两组的差异被完全掩盖；
真正撞上 K=64 要到片元 14.7M（64 个/像素）之后，本场景发生在 1800 步附近。
